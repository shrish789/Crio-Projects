...
public class RestaurantServiceMockitoTestStub {
  ...
  public Restaurant[] initializeRestaurantObjects() throws IOException {
    String fixture =
  ...
  public void testFindNearbyWithin5km() throws IOException {
    ...
    Restaurant[] restaurants = initializeRestaurantObjects();
    restaurant1 = restaurants[0];
    restaurant2 = restaurants[1];
    restaurant3 = restaurants[2];
    restaurant4 = restaurants[3];
    ...
    when(restaurantRepositoryServiceMock.findAllRestaurantsCloseBy(any(Double.class), any(Double.class),
        eq(LocalTime.of(3, 0)), eq(5.0))).thenReturn(restaurantList);
    ...
    verify(restaurantRepositoryServiceMock, times(1)).findAllRestaurantsCloseBy(any(Double.class), any(Double.class),
  ...
  public void testFindNearbyWithin3km() throws IOException {
    ...
    // TODO: CRIO_TASK_MODULE_MOCKITO
    // Initialize these two lists above such that I will match with the assert
    // statements
    // defined below.

    lenient().doReturn(restaurantList1).when(restaurantRepositoryServiceMock).findAllRestaurantsCloseBy(eq(20.0),
        eq(30.2), eq(LocalTime.of(3, 0)), eq(5.0));
    ...
    lenient().doReturn(restaurantList2).when(restaurantRepositoryServiceMock).findAllRestaurantsCloseBy(eq(21.0),
        eq(31.1), eq(LocalTime.of(19, 0)), eq(3.0));
    ...
    // TODO: CRIO_TASK_MODULE_MOCKITO
    // Call restaurantService.findAllRestaurantsCloseBy with appropriate parameters
    // such that
    // Both of the mocks created above are called.
    // Our assessment will verify whether these mocks are called as per the
    // definition.
