...
public class AlphavantageService implements StockQuotesService {
  ...
  public List<Candle> getStockQuote(String symbol, LocalDate from, LocalDate to) 
    ...
    Map<LocalDate, AlphavantageCandle> map = null;
    ...
    try{
      map = objectMapper.readValue(response, AlphavantageDailyResponse.class).getCandles();
    } catch (Exception e) {
      throw new StockQuoteServiceException("Invalid Parsing");
    }
    ...
    for(LocalDate date = from; !date.isAfter(to); date = date.plusDays(1)) {
      ...
      if(candle != null) {
        candle.setDate(java.sql.Date.valueOf(date));
